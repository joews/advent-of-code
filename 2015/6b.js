...
const inc = (x, y, value) => { state[idx(x, y)] = Math.max(0, get(x, y) + value) };
...
const setRange = (op, [x0, y0], [x1, y1]) => {
  for (let x = x0; x <= x1; x ++) {
    for (let y = y0; y <= y1; y ++) {
      if (op === "turn on") inc(x, y, 1);
      else if (op === "turn off") inc(x, y, -1);
      else if (op === "toggle") inc(x, y, 2)
...
const count = state.reduce((sum, cell) => sum + cell, 0);
console.log(count);
